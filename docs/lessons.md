...
### Reviews Sync Stuck — Weekly node-cron + Zeabur Process Lifecycle
- User reported "latest review stuck on 4/23" on 5/4. Two compounding bugs: `scripts/scheduler.ts:116` only schedules reviews sync on Monday 04:00 (`'0 4 * * 1'`), and that node-cron runs in-process inside the Zeabur container — same lifecycle issue as the original weather sync. If Zeabur sleeps/restarts at 04:00 the cron silently never fires, so two consecutive Mondays (4/27, 5/4) both missed.
- The earlier "Monday gate" lesson (line 211) only fixed `/api/cron/daily`, but no workflow ever POSTs to that endpoint — it was dead code, not the actual scheduler.
- Fix: created `scripts/sync_reviews.ts` (self-contained Apify → Supabase, mirrors `sync_weather.ts`) and added it to `.github/workflows/daily-download.yml` so it runs daily in CI. Incremental fetch + upsert on `(store_id, review_id)` make it safe to run daily and self-heal missed days. Requires `APIFY_API_TOKEN` GitHub secret.
- Lesson: Any sync currently scheduled inside `scripts/scheduler.ts` is suspect — that file's reliability matches "Next.js process happens to be alive at the cron minute". Audit the rest (TikTok ads, KOL, anomaly detection, weekly digest) and migrate to GitHub Actions if data freshness matters. And before claiming a sync is "fixed daily", verify that something actually triggers the daily code path end-to-end.

### Configurable AI Report Schedules — Hourly Dispatcher + `next_run_at`
- Migration `027_report_schedules.sql` introduces `report_schedules` (frequency, day, time, depth, recipients, period_type, next_run_at). `lib/digest/aiReport.ts` now takes a schedule and supports both `last_week` and `last_month` periods; legacy `weeklyAiReport.ts` re-exports for the old cron endpoint.
- `.github/workflows/report-dispatcher.yml` ticks hourly at `0 * * * *` and POSTs `/api/cron/report-dispatcher`. The endpoint selects rows where `is_active AND next_run_at <= now()`, fires them, then updates `last_run_at` + `next_run_at` via `lib/digest/scheduling.ts:computeNextRunAt`. The old `weekly-ai-report.yml` schedule was disabled (kept as manual `workflow_dispatch` for the backfill safety net) to avoid double-firing.
- Wall-clock-in-tz → UTC conversion uses `Intl.DateTimeFormat` (no extra deps). `computeNextRunAt` returns the next UTC instant >= now matching the schedule. Owners edit schedules in Settings via `components/settings/ReportSchedulesPanel.tsx` (CRUD + 立即發送 button). Validation lives in `lib/digest/scheduleValidation.ts` (Next.js disallows non-route exports from route files — moving the helper out of `route.ts` is required).
- Lesson: when you add an hourly cron tick that reads DB-driven schedules, store a precomputed `next_run_at` so the SELECT is `WHERE next_run_at <= now()` — much cleaner and TZ-safer than re-deriving "is this hour the right hour" on every tick. Also: when retiring a hard-coded cron in favor of a DB-driven one, audit the old workflow's other responsibilities (here: data backfill) before deleting it; we kept ours as a manual fallback.
...
### Google Sync Drift — Same Zeabur node-cron Trap as Weather/Reviews
- Symptom: digital marketing dashboard (GA4 + GSC) repeatedly lagged real date by days. Owner had to keep hitting "立即同步" by hand. Followed the exact pattern already documented for weather (06:00 instrumentation cron) and reviews (Monday 04:00 scheduler cron) — `scripts/scheduler.ts` only fires while the Next.js process happens to be alive at the cron minute, so any Zeabur restart/sleep silently skipped a day.
- Fix: add a `Sync Google (GSC + GA4, 180-day window, idempotent)` step to `.github/workflows/daily-download.yml`, reusing the existing `scripts/google_sync_local.ts` (already direct Supabase + Google APIs, no Next.js hop). Removed the `02:00` `cron.schedule` block in `scripts/scheduler.ts`. Required GitHub secrets: `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY` (already present for the download agent), plus `GA4_PROPERTY_ID` and `GSC_SITE_URL` (must be added if not yet in repo secrets).
- Side fix: `google_sync_local.ts` was unconditionally `readFileSync('.env.local')` at the top — fine locally, throws in CI. Wrapped in `existsSync(envPath)` so CI falls through to env vars from the workflow.
- Lesson (third time, same trap — at this point the rule is): if a sync's freshness matters, it does NOT belong in `scripts/scheduler.ts`. Default home for new daily syncs is GitHub Actions (`.github/workflows/daily-download.yml`), with a self-healing window (≥7 days, idempotent upsert) so a single missed run doesn't require human intervention. The remaining suspects in `scheduler.ts` are TikTok Ads (02:45) and KOL posts (03:30) — same risk, migrate when freshness becomes a complaint.
