...
import { detectPlatform, isDetectionError } from '@/lib/kol/platformDetector'
import { scrapePost } from '@/lib/kol/apifyKolClient'

export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const { collaboration_id, post_url, store_id } = body
    ...
    // Insert post record with pending status
    ...
    scrapePost(detection.platform, detection.url)
      .then(async (result) => {
        await supabase
          .from('kol_posts')
          .update({
